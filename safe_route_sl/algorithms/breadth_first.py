"""Breadth-first search for SafeRouteSL."""
    ...
    def search(self, graph: nx.Graph, configuration: SearchConfiguration) -> SearchResult:
        ...
        steps = [
            make_initial_step(
                ...
                explanation=f"The search starts at {start_node}. The frontier contains only the start node.",
            )
        ]
        ...
        if start_node == goal_node:
            ...
            goal_step = make_step(
                ...
                maximum_frontier_size=1,
                goal_found=True,
                explanation=f"BFS reached the goal immediately at {start_node}.",
            )
        ...
        while frontier:
            ...
            if current_node == goal_node:
                ...
                goal_step = make_step(
                    ...
                    goal_found=True,
                    explanation=f"BFS removed {current_node} from the front of the FIFO queue and found the goal.",
                )
            ...
            step = make_step(
                ...
                maximum_frontier_size=maximum_frontier_size,
                explanation=(
                    f"BFS removed {current_node} from the front of the FIFO queue. "
                    + (
                        f"It added {', '.join(generated_nodes)} to the end of the queue. "
                        if generated_nodes
                        else "It did not add any new neighbours. "
                    )
                    + (
                        f"Skipped {', '.join(skipped_nodes)} because they were already explored or queued."
                        if skipped_nodes
                        else ""
                    )
                ).strip(),
        ...
        failure_step = make_step(
            ...
            goal_found=False,
            explanation=(
                f"The frontier is empty. No traversable route exists between {start_node} and {goal_node} under the current scenario."
            ),
        )
